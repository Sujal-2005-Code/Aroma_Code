...
import { getMentorMessages, sendMentorMessage, type MentorMessage } from "@/lib/api/resources";
...
export default function MentorPage() {
  ...
  const send = (content = input) => {
    ...
    sendMentorMessage(content).then((response) => setMessages((items) => [...items, response])).catch((cause) => { setMessages((items) => items.filter((item) => item.id !== optimistic.id)); setError(cause instanceof Error ? cause.message : "Could not send message."); }).finally(() => setSending(false));
  };
