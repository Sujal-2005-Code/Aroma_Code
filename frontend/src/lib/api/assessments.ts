...
export const submitAssessment = (data: { student_id?: string; assessment_id: string; session_id: string; answers: Record<string, SubmissionAnswer> }) => isMock(data.assessment_id) ? Promise.resolve({ submission_id: "mock-sub", assessment_id: data.assessment_id, student_id: "mock", total_score: 80, result: "PASS", submitted_at: new Date().toISOString() } as unknown as SubmissionResult) : api<SubmissionResult>("/submissions", { method: "POST", body: JSON.stringify(data) });
export const getAssessmentResult = (assessmentId: string) => isMock(assessmentId) ? Promise.resolve({ submission_id: "mock-sub", assessment_id: assessmentId, student_id: "mock", total_score: 80, result: "PASS", submitted_at: new Date().toISOString() } as unknown as SubmissionResult) : api<SubmissionResult>(`/student/result/${assessmentId}`);
export const generateAIAssessment = (data: { skill: string; topic: string; level: string; assessment_type?: string; num_questions?: number }) => api<{ assessment_id: string; questions: Array<{ id: number; question: string; options: string[]; topic?: string; difficulty?: string }> }>('/ai-assessment/generate', { method: 'POST', body: JSON.stringify(data) });
export const submitAIAssessment = (assessmentId: string, answers: Array<{ questionId: number; selectedAnswer: string | null }>) => api<any>(`/ai-assessment/${assessmentId}/submit`, { method: 'POST', body: JSON.stringify({ answers }) });
export const getAIAssessmentResult = (assessmentId: string) => api<any>(`/ai-assessment/${assessmentId}/result`);
